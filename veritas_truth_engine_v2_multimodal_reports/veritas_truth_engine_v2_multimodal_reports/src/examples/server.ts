...
const server = http.createServer(async (req: any, res: any) => {
  ...
  const body = `
    ...
    <div class="card">
      ...
      <p><strong>Truth score:</strong> ${result.posteriorTruthScore.toFixed(4)}</p>
      <p><strong>Release decision:</strong> ${result.releaseState}</p>
      <p><strong>Review reasons:</strong> ${result.explanation.reviewReasons.join(", ") || "none"}</p>
    </div>
    <div class="card">
