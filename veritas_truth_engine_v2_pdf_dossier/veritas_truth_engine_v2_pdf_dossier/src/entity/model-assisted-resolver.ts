import { CanonicalEntity, CanonicalizationCandidate, EntityMention } from "./types";
import { normalizeEntityName, scoreEntitySimilarity } from "./normalizer";
...
export function resolveMentionWithHeuristics(
  ...
  const candidates: CanonicalizationCandidate[] = canonicalEntities
    .map((entity) => {
      const score = scoreEntitySimilarity(normalized, normalizeEntityName(entity.name));
      return {
        entityId: entity.id,
