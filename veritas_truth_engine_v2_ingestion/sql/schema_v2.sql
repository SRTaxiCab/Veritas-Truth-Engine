...
create table if not exists ingested_documents (
    ...
    ingestion_status text not null default 'completed',
...
create index if not exists idx_document_chunks_doc_idx on document_chunks(ingested_document_id, chunk_index);
...
create table if not exists extraction_runs (
    ...
    extraction_mode text not null default 'heuristic_v1',
...
create index if not exists idx_claim_occurrences_chunk on claim_occurrences(document_chunk_id);
